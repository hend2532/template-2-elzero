      ...
      <div className="flex items-center gap-4">
        ...
        <nav
          className={`absolute md:relative px-8 md:px-0 bg-[#171515b9] md:bg-transparent top-[100px] md:top-0 left-0 w-full md:w-auto md:flex
            ${isOpen ? "flex" : "hidden"}`}
        >
          <ul className="flex flex-col md:flex-row md:gap-6 font-medium items-center w-full">
            {navLinks.map((link) => (
              <li key={link.href} className="w-full md:w-auto text-center">
                <a
                  href={link.href}
                  className="block px-2 py-[39px] border-b-[3px] border-b-transparent hover:text-[#19c8fa] hover:border-b-[#19c8fa]"
                >
                  {link.label}
                </a>
              </li>
